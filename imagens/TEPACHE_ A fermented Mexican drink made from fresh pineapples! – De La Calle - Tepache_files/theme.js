...
Line 10416:0 Transforming const to the configured target environment ("es5") is not supported yet
Line 10417:0 Transforming const to the configured target environment ("es5") is not supported yet
Line 10428:0 Transforming let to the configured target environment ("es5") is not supported yet
Line 10429:0 Transforming let to the configured target environment ("es5") is not supported yet
Line 10430:0 Transforming let to the configured target environment ("es5") is not supported yet
Line 10431:0 Transforming let to the configured target environment ("es5") is not supported yet
Line 10432:0 Transforming let to the configured target environment ("es5") is not supported yet
...
theme.Header = (function () {
  ...
  function init() {
    cacheSelectors();
    styleDropdowns(document.querySelectorAll(selectors.siteNavHasDropdown));//TODO: check error in the browser console
...
theme.Cart = (function () {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _createCart: function (state) {
      ...
      var cartRowLineItems = this.container.querySelectorAll(selectors.cartItem);
      cartRowLineItems.forEach(function (e) {
        e.remove();
      });
      // cartTable.innerHTML = '';

      this._createLineItemList(state).forEach(function (lineItem) {
        cartTable.prepend(lineItem);
      });
...
theme.Product = (function () {
  function Product(container) {
    ...
    this.zoomPictures = [];
    this.ajaxEnabled = container.getAttribute('data-ajax-enabled') === 'false';
    ...
    this.selectors = {
      ...
      cartCount: '[data-cart-count]',
      cartCountBubble: '[data-cart-count-bubble]',
      cartPopupLevelUp: '[data-cart-level-up]',
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _onBodyClick: function (event) {
      ...
      if (target !== this.cartPopupWrapper && !target.closest(this.selectors.cartPopup) && !target.closest(this.selectors.cartPopupLevelUp)) {
        this._hideCartPopup(event);
      }
    },
...
//    * */
//
// var featureTitle = document.getElementsByName('.features-section__title h2');
//
//
// if (featureTitle.length) {
//   var featureTitleText = featureTitle.innerText;
//
//   var replacedHtml = featureTitleText.replace(/(\d+):(\d+)/g, '$1:<span>$2</span>');
//
//   featureTitle.innerHTML(replacedHtml);
//
...
$(".hero-page .btn, .subscription-info .btn").on('click', function (event) {
  event.preventDefault();
  var hash = $(".subscription-product");
  $('html, body').animate({
...
$('.blog-list-view.shuffled').randomize();

const phoneInputField = document.querySelector("#k_id_modal_phone-number");
...
$(document).ready(function () {
  $('#k_id_modal_phone-number').inputmask({
    mask: "999 999 9999"
  });
});

let recoverPasswordBtn = document.getElementById('RecoverPassword');
let recoverPasswordBtnHide = document.getElementById('HideRecoverPasswordLink');
let loginSection = document.getElementById('CustomerLoginForm');
let recoverSection = document.getElementById('RecoverPasswordForm');
let header = document.getElementById('shopify-section-header');

recoverPasswordBtn.addEventListener('click', function (e) {
  e.preventDefault();
  recoverSection.classList.remove('hide');
  loginSection.classList.add('hide');
  header.scrollIntoView();
});

recoverPasswordBtnHide.addEventListener('click', function (e) {
  e.preventDefault();
  loginSection.classList.remove('hide');
  recoverSection.classList.add('hide');
  header.scrollIntoView();
});